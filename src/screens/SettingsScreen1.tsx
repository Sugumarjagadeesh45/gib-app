...
useEffect(() => {
 ...
 const handleSubmit = async () => {
  if (!selectedUser) return Alert.alert('Please select a user.');
  if (!businessAmount) return Alert.alert('Please enter amount.');
  ...
  try {
    const res = await axios.post(
      'https://www.giberode.com/giberode_app/thanksnote_api.php',
      {
        ...
        to_phone: selectedUser.phone,
        business_amount: businessAmount,
        given_take: givenTake.toLowerCase(),
      },
  ...
  if (loading) {
    return (
      <View style={styles.centered}>
        <ActivityIndicator size="large" color="#098241" />
